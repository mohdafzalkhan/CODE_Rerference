...
if($type=='register'){
	...
	if($check>0){
		...
		mysqli_query($con,"insert into users(name,email,mobile,password,status,email_verify,added_on,rand_str,referral_code,from_referral_code) values('$name','$email','$mobile','$new_password','1','0','$added_on','$rand_str','$referral_code','$from_referral_code')");
	...
	if($check>0){	
		...
		if($email_verify==1){
			if($status==1){
				...
				mysqli_query($con,"update users set password='$new_password' where id='$id'");
				$html=$rand_password;
